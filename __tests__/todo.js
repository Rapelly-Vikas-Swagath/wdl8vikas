...
describe("Testing My Todo App ", () => {
  ...
  test("Updating Completed Field: ", async () => {
    const res = await agent.get("/");
    const csrfToken = extractCsrfToken(res);
    await agent.post("/todos").send({
      title: "Start Preperation for Mids",
      dueDate: new Date().toISOString(),
      completed: false,
      _csrf: csrfToken,
    });
 ...
 //Test
  ...
  test("Completion Marking Using Mouse Pointer", async () => {
  ...
  test("Mark Incomplete Based on ID", async () => {
    let res = await agent.get("/");
    let csrfToken = extractCsrfToken(res);
    await agent.post("/todos").send({
  ...
  //Test
  test("Delete Todo Based on ID", async () => {
    let res = await agent.get("/");
    let csrfToken = extractCsrfToken(res);
    await agent.post("/todos").send({
      title: "Vacation Completion",
      dueDate: new Date().toISOString(),
      completed: false,
      _csrf: csrfToken,
    });
